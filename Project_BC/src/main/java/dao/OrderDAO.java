	...
	public ArrayList<OrderDetailBean> selectOrderProductList(int order_num) {
		...
		try {
			...
			while (rs.next()) {
				...
				if (rs1.next()) {
					orderProduct.setProduct_name(rs1.getString("product_name"));
					orderProduct.setSeller_id(rs1.getString("seller_id"));
					orderProduct.setProduct_price(rs1.getInt("product_price"));
					orderProduct.setProduct_discount(rs1.getInt("product_discount"));
					...
					if (rs2.next()) {
						orderProduct.setSname(rs2.getString("Sname"));
					}
				}
