...
import svc.memberSvc.FindCustomerInfoService;
import svc.memberSvc.FindIdProService;
import svc.memberSvc.FindSellerInfoService;
import vo.ActionForward;

public class FindPwdProAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		System.out.println("여기 3"+findGrade);

		
		if(findGrade == 3) {
		email = customerService.FindCustomerEmail(id);
		customerService.FindCustomerPwd(id);
		
		String emailSub = "****";
		String subEamil = email.substring(4);
		
		String realEmail = emailSub+subEamil;
		
		System.out.println(realEmail);
		HttpSession session = request.getSession();
		session.setAttribute("pId", realEmail);
		
		String content = "<a href=http://localhost:8091/Project_BC/changePwd.me?id=" + id +"><b>비밀번호를 변경하실려면 다음 링크를 클릭하세요.</b></a><br>";
		...
		try {
			...
			System.out.println("셀러 등급찾기 성공");
			email = sellerService.FindSellerEmail(id);
			customerService.FindCustomerPwd(id);
			
			String emailSub = "****";
			String subEmail = email.substring(4);
			

			String realEmail = emailSub+subEmail;
			
			System.out.println(realEmail);
			HttpSession session = request.getSession();
			session.setAttribute("pId", realEmail);
			
			String content = "<a href=http://localhost:8080/Project_BC/changePwd.me?id=" + id +"><b>비밀번호를 변경하실려면 다음 링크를 클릭하세요.</b></a><br>";
			response.setContentType("text/html; charset=UTF-8");
			PrintWriter out = response.getWriter();
		...
		if(findGrade ==  0) {
			...
			PrintWriter out = response.getWriter();
			out.println("<script>");
			out.println("alert('아이디와 이름을 다시 입력해주세요')");
			out.println("</script>");
		}
